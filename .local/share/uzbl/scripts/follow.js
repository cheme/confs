...
uzbl.follow.reDrawHints = function(elements, len) {
	...
	if (len == 0)
        var label  = uzbl.follow.getTextLabel(el); 
	else
...
//get label TODO get rid of the persistence: a bit useless
uzbl.follow.getTextLabel = function(el) {
		if (el.hasAttributeNS("pers", "hint")){
 return el.getAttributeNS("pers","hint");
}
var result = "";
                if (el instanceof HTMLInputElement || el instanceof HTMLSelectElement || el instanceof HTMLTextAreaElement)
                    return el.value;
                else if (el.firstElementChild instanceof HTMLImageElement && /^\s*$/.test(el.textContent))
                    result = (el.firstElementChild.alt || el.firstElementChild.title);
                else
                    result = el.textContent.toLowerCase();
 el.setAttributeNS("pers","hint", result);
return result;
}
...
uzbl.follow.followTextContent = function(str) {
    ...
    if(matching.length == 1)
        ...
        //ECH added the two following lines
        this.reDrawHints(matching,0 );
}

uzbl.follow.followLinks = function(str) {
    var elems    = uzbl.follow.getInterestingElements();
    var leftover = uzbl.follow.findMatchingHintId(elems, str);
    //ECH added two following lines
    if(leftover.length == 0){
	return uzbl.follow.followTextContent (str); 
    }
